...
4. Run `pnpm run build` (which re-runs codegen, then `tsc -p
   ...
   `as const` tuple type.
5. Commit both the JSON in `network/` and the regenerated files under
   `src/generated/` in the same commit. The PR gate runs
   `codegen-drift-check` (regenerate, then `git diff --exit-code`) and
   fails when the two disagree. `.gitattributes` marks
   `src/generated/**` as `linguist-generated` so GitHub collapses it
   in the PR diff by default.
...
`network/`; the typed TS modules are codegenned.
