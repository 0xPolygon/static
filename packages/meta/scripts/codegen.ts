...
//
// JSON in `network/` is the source of truth. The committed
// `src/generated/` tree is regenerated by this script; the
// `codegen-drift-check` PR gate fails when the two disagree.
//
...
const stats: Stats = { abi: 0, info: 0, networks: 0, mismatches: [] };
...
function readTsLiteral(tsPath: string, exportName: string): unknown {
  ...
  for (const network of listDirs(chainPath)) {
    ...
    for (const type of types) {
      ...
      for (const fileName of listFiles(typePath)) {
        ...
        const srcJson = JSON.parse(readFileSync(inPath, 'utf8')) as { abi?: unknown };
...
// 3. Top-level network registry: network/networks.json
{
  ...
  const networks = JSON.parse(readFileSync(networksPath, 'utf8')) as unknown;
